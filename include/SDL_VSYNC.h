...
#define NUMBERFRAMESONSAMEPLACE  2   // nombre de fois que l'on redessine chaque sprite avant de le deplacer
...
typedef struct {
    ...
    //position coin sup gauche oe est affichee la texture dans la fenetre (renderer)
    int posX;
    int posY;

    //dist du prochain affichage   (posXarriveeA - PosXdepartB)/NBIMAGESPOURLEDEPLACEMENT_A_VERS_B
    int incX;
    int incY;
}LTexture;
